...
exports.createRestaurantSchedule = async (req, res) => {
    try{
        ...
        if(!schedule) {
            return sendResponse(res, 500, {message: "Error al crear el Horario"});
        }

        return sendResponse(res,201, {data: schedule});

    }catch(error) {
        return sendResponse(res,500, {message: error.errors[0].message})
    }
}
